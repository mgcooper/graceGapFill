...
if numel(Mlist)*numel(Klist) > 1 % cross validation is required
    ...
    for  iK = 1:numel(K)
        if iK == 1
%             f = waitbar(iK/numel(K),sprintf('wait for cross validation: %d%%', ...
%                     round(iK/numel(K)*100)));
        else
%             waitbar(iK/numel(K),f,sprintf('wait for cross validation: %d%%', ...
%                     round(iK/numel(K)*100)));
        end
%         fprintf('wait for cross validation: %d%%',round(iK/numel(K)*100));
        % mgc I commented out the waitbar, added the fprintf, but then
        % decided to move that to bfra_fillGRACE so i can display the grid
