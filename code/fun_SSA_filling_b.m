...
% mgc I commented out the waitbar, added the fprintf, but then moved that
% to bfra_fillGRACE so i can display the grid cell being processed
         
         % fprintf('wait for cross validation: %d%%',round(iK/numel(K)*100));
         
%          msg = sprintf('wait for cross validation: %d%%',round(iK/numK*100));
%          if iK == 1
%             f = waitbar(iK/numK,msg);
%          else
%             waitbar(iK/numK,f,msg);
%          end
